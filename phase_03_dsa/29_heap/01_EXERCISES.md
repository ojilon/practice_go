...
> Work in THIS folder, `package heapx`. Run via root `main.go` (temporary import). Verifier checks THESE files, not `main.go`.

## Must implement (checked by verifier)
- **E1 heap.go** — generic Heap[T any] with less func; Push/Pop/Top/Len/IsValid().
- **E2 tasks demo** — E2 tasks demo: tasks with priority popped in order.
...
- Package clause MUST be `package heapx`.
