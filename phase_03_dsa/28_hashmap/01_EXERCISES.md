# hashmap — Hash Table From Scratch — Exercises

> Work in THIS folder, `package hashmap`. Run via root `main.go` (temporary import). Verifier checks THESE files, not `main.go`.

## Must implement (checked by verifier)
- **E1 hashmap.go** — New(cap), Put/Get/Delete/Len/LoadFactor with resize.
...
- Package clause MUST be `package hashmap`.
