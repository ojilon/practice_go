# config — Env/Flags/Defaults — Exercises

> Work in THIS folder, `package config`. Run via root `main.go` (temporary import). Verifier checks THESE files, not `main.go`.

## Must implement (checked by verifier)
- **E1 config.go** — New(defaults), Set, GetString(key,def), GetInt, GetBool, LoadFromEnv(prefix).
- **E2 precedence.go** — demo proving env overrides default (t.Setenv in demo func).
...
- Package clause MUST be `package config`.
