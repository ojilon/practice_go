# event — EventBus — Exercises

> Work in THIS folder, `package event`. Run via root `main.go` (temporary import). Verifier checks THESE files, not `main.go`.

## Must implement (checked by verifier)
- **E1 bus.go** — Bus with Subscribe/Publish/Unsubscribe/Close; concurrent-safe.
- **E2 demo once.go** — E2 demo once.go: func Once(b *Bus, topic string) any blocking for first event (with timeout variant).
...
- Package clause MUST be `package event`.
