# set — Generic Set — Exercises

> Work in THIS folder, `package set`. Run via root `main.go` (temporary import). Verifier checks THESE files, not `main.go`.

## Must implement (checked by verifier)
- **E1 set.go** — full API + Union/Intersect/Diff/Clone/Clear.
- **E2 dedup.go** — func Dedup[T comparable]([]T) []T using Set preserving order.
...
- Package clause MUST be `package set`.
