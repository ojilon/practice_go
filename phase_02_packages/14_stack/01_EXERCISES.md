# stack — LIFO — Exercises

> Work in THIS folder, `package stack`. Run via root `main.go` (temporary import). Verifier checks THESE files, not `main.go`.

## Must implement (checked by verifier)
- **E1 stack.go** — generic Stack with full API + var ErrEmpty.
- **E2 balanced.go** — func Balanced(s string) bool using your Stack[rune].
...
- Package clause MUST be `package stack`.
