# queue — FIFO + Deque — Exercises

> Work in THIS folder, `package queue`. Run via root `main.go` (temporary import). Verifier checks THESE files, not `main.go`.

## Must implement (checked by verifier)
- **E1 queue.go** — generic Queue with Enqueue/Dequeue/Peek/Len.
- **E2 deque.go** — Deque with 4 ops; func LevelOrder demo comment.
...
- Package clause MUST be `package queue`.
