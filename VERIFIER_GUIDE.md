...
### Phase 6 systems (fully specified — same verify-one-folder flow)
- `43_wal_seglog` (`walseglog`): `Open/Append/Read/Sync/Close`, `LowestOffset/HighestOffset/Stats`, `TruncateBefore`, streaming `Replay`, `Subscribe` XOR `WaitFor`; sentinels `ErrClosed/ErrCorrupt/ErrBadOffset/ErrBadHeader/ErrTooLarge`; banned `encoding/gob`, `encoding/json` (record path), bbolt/pebble/sqlite/kafka clients; QUESTIONS Q2–Q4 show real reopen counts + kill/truncate + sync bench numbers; `DemoReuse44Style` proves Replay→map rebuild.
- `44_kv_store` (`kvstore`): `Open/Put/Get/Delete/Close/Stats`, atomic `Batch`, `PutWithTTL/GetWithMeta`, crash-safe `Compact() (CompactionStats,error)`; `ErrNotFound/ErrLocked/ErrClosed/ErrTooLarge/ErrBadKey`; MUST import `43_wal_seglog` + logger/config/cache/scheduler/taskpool; QUESTIONS show reopen survivors + compaction before/after shrink + bench puts/s+gets/s; `README.md` with arch + durability contract present (note if missing but don't fail symbols for it — mark PARTIAL).
- `45_http_server_from_tcp` (`httpraw`): `ParseRequest`, `ResponseWriter` (WriteHeader/Write/Header), `Router` (Handle/ServeHTTP/Use + `:param` + 404/405), `Server` (NewServer/ListenAndServe/Shutdown/Addr), `ServeKV` + `ServeFiles` (traversal rejected); banned `net/http` in impl (client use in demo_cli only) + gin/echo/chi/fasthttp; QUESTIONS show real curl/Go-client transcripts + attack 4xx + slow-client 408 + load req/s.
- `46_resp_rpc` (`resprpc`): RESP `Value/Marshal/NewReader/ReadValue` + `ErrFrameTooLarge/ErrBadFrame/ErrTruncated`, `ServeRESP/Addr/Close` (PING/ECHO/SET/GET/DEL/INCR/EXISTS/COMMAND), RPC `Envelope` marshal/unmarshal + `RPCServer` (Register/ServeConn/ServeAddr) + `RPCClient` (DialRPC/Call/Close) + `ErrUnknownMethod/ErrTimeout/ErrCorrupt/ErrClosed`; banned redis libs, net/rpc, grpc, gob/json on wire; QUESTIONS show codec 0-mismatch + pipelining bytes + atomic INCR exact + RPC multiplex/timeout + Raft-shaped round-trip.
- `47_raft_lite` (`raftlite`): `State/Entry/Committed/NodeOptions/DefaultNodeOptions`, `ErrNotLeader{LeaderHint}/ErrClosed/ErrNoLeader`, `NewNode/Start/Stop/State/Term/Leader/CommitIndex/Submit/ApplyCh`; vote/append over 46 (`raft.requestVote`/`raft.appendEntries`) with conflict-hint repair; simulator + `StartTCPCluster` + kill/partition/heal; demos Election(20×1-leader)/StepDown/Replicate/Catchup/TCPCluster/Chaos(4-scenario)/BenchRaft/CLI; banned etcd/raft, hashicorp/raft, net/http-RPC, gob entries; QUESTIONS show election + replication + TCP/bench + 3× chaos table + paper-argument sentences; `RAFT_NOTES.md`+`README.md` present (missing → PARTIAL).
- `48_paper_reimpl` (`paperreimpl`): `Track()` in {dynamo,mapreduce,gfs} + `EvalRow/FailureResult` + track errors; ONE track core complete (Dynamo clocks+Put/Get-siblings / MR Run / GFS trio+TestCluster) + `DemoEval/DemoFailure/ReuseMap` + `Demo()`/`DemoCLI` + matching correctness demo (Conflict/FaultTolerance/Failover); ≥6 reuses named in `ReuseMap()` + Q1 (else PARTIAL); banned riak/hadoop/hdfs + raft libs + net/http-internal-RPC + gob; QUESTIONS show correctness PASS + eval table + failure matrix + related-work note + race/demo commands; `PAPER_NOTES.md`+`README.md`+`RETRO.md` present (missing → PARTIAL).
...
- **Reuse check (Phase 6):** `43` (logger+config+io-cite), `44` (43+logger+config+cache+scheduler+taskpool), `45` (logger+config+jsonlite+taskpool+44), `46` (44+taskpool+logger+config), `47` (43+46+logger+config+scheduler/taskpool), `48` (≥6 incl. 43+44+46 spine). Same import-or-cite rule; `48` with <6 = PARTIAL even if code passes. `48` also requires `PAPER_NOTES.md`+`README.md`+`RETRO.md` existence (missing → PARTIAL); `47` requires `RAFT_NOTES.md`+`README.md`; `44/45/46` require `README.md` (missing → PARTIAL, not FAIL).
