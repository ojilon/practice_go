# mini-os — Files, Args, Signals — Exercises

> Work in THIS folder, `package minios`. Run via root `main.go` (temporary import). Verifier checks THESE files, not `main.go`.

## Must implement (checked by verifier)
- **E1 fileutil.go** — Cat(path)(string,error), CopyFile(src,dst) error, ListDir(path) []string.
- **E2 sig.go** — func RunUntilInterrupt(work func(ctx)) with signal handling.
...
- Package clause MUST be `package minios`.
